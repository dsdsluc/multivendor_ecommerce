              ...
              <div key={variant.id} className="flex flex-col gap-y-2 group">
                <div className="relative cursor-pointer p-2">
                  <Image
                    ...
                    width={1000}
                    height={1000}
                    className="w-56 h-56 rounded-md object-cover shadow-sm"
                  />
                  <Link
                    href={`/dashboard/seller/stores/${row.original.store.url}/products/${row.original.id}/variants/${variant.id}`}
                  >
                    <div className="absolute inset-0 z-0 rounded-sm bg-black/50 transition-all duration-150 hidden group-hover:block">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2 max-w-72 mt-1">
                        {variant.sizes.map((size) => (
                          <span
                            key={size.size}
                            className="w-fit px-2 py-1 rounded-md text-[11px] font-medium border bg-muted text-muted-foreground"
                          >
